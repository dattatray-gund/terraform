...
func renderPlan(plan *plans.Plan, schemas *terraform.Schemas, view *View) {
	haveRefreshChanges := plan.DriftChanges != nil && len(plan.DriftChanges.Resources) > 0
	if haveRefreshChanges {
		renderChangesDetectedByRefresh(plan.DriftChanges, schemas, view)
		switch plan.UIMode {
...
func renderChangesDetectedByRefresh(changes *plans.Changes, schemas *terraform.Schemas, view *View) {
	view.streams.Print(
