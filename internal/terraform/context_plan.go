...
func (c *Context) planWalk(config *configs.Config, prevRunState *states.State, rootVariables InputValues, opts *PlanOpts) (*plans.Plan, tfdiags.Diagnostics) {
	...
	driftChanges, driftDiags := c.driftChanges(config, prevRunState, priorState)
...
func (c *Context) driftChanges(config *configs.Config, oldState, newState *states.State) (*plans.Changes, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
		...
		for _, rs := range ms.Resources {
			...
			for key, oldIS := range rs.Instances {
				...
				change := &plans.ResourceInstanceChange{
					...
					ActionReason: plans.ResourceInstanceDriftDetected,
				}
				...
				cs.AppendResourceInstanceChange(changeSrc)
			}
		}
	}
